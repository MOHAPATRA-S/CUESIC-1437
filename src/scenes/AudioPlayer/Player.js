...
export default  Player = (props)=> {
  ...
  const seek=(time)=> {
    console.log("useRef.audioElement",audioElement)
    time = Math.round(time);
    ...
    return (
     ...
     {/* }  */}
        <Drop_Down_Modal_forall
          ...
          onPressno={() => { setState({ ...inbuiltstate,logoutmodal: false }); Actions.tabBar() }}
          onPressyes={() => { setState({ ...inbuiltstate,logoutmodal: false, savemodal: true }) }}
        />
        <AppModal
          animationType='fade'
          visible={inbuiltstate.savemodal}>
          <TouchableIn onPressIn={() =>setState({...inbuiltstate, savemodal: false })} style={style.mainContainer} >
            <AppView style={style.Modalcontainer}>
              <AppView
                style={{
                  width: width * 0.8,
